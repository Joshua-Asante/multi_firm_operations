...
2026-05-17 lock MC anchor (4-strategy + dd_protection C2, BT-OFF + static-equity canonical, current canonical):
* Pepperstone 4-strategy (G 0.34% / DJ30 v4.5 1.00% / A 1.50% / NAS v1 0.40%, dd_protection 1.5%/0.40×, 10K × 3 seeds): **97.56% pass / 0.40% bust (0.00% daily + 0.40% static) / 2.04% timeout**, p99 DD 4.79%, median days-to-pass 23. **Bust attribution**: striker 42.9% / guardian 25.2% / aegis 22.7% / NAS 9.2%. NAS remains the lowest contributor consistent with the diversification thesis. Reproducible under `python portfolio_mc.py --panel pepperstone`. `tests/test_mc_anchors.py` pins these. Lock criteria (bust <1%, p99 DD <5%) — both pass with margin. **Anchor update from 2026-05-08 BT-ON anchor (98.09/0.36/4.73)** — not a re-lock event; methodology change replaces BT-ON compounded Pepperstone panels with BT-OFF panels under FXIFY static-$200K sizing convention (Pine sizing uses `strategy.equity` which overstates live execution; corrected via per-strategy `Net P&L USD × (INITIAL / equity_at_entry)`). Per-strategy static/compounded Net ratios: Guardian 54% / Aegis 73% / DJ30 67% / NAS 58%. Pass drops 0.53 pp, bust edges up 0.04 pp, median days returns to 23 (was 22 under BT-ON C2) — all within historical range and well under lock criteria. See [`data/reconciles/2026-05-17_guardian_bt_off_static_canonical.md`](data/reconciles/2026-05-17_guardian_bt_off_static_canonical.md) for the full methodology change documentation.
* OANDA pattern-spotting proxy at C2 (3-strategy, DJ30 still v4.4): **96.23% pass / 0.69% bust / p99 DD 4.91%**, median days-to-pass 25. Both lock criteria clear with thinner margin than Pepperstone, consistent with OANDA's pattern-spotting role. Reproducible under `python portfolio_mc.py --panel oanda`. OANDA panels remain BT-ON for this methodology cascade (out of scope per 2026-05-17 decision — OANDA is pattern-spotting only, not used for canonical reconfiguration).

Prior anchors (historical):
* 2026-05-08 4-strategy BT-ON at C2 (1.5%/0.40×): **98.09% pass / 0.36% bust / 4.73% p99 DD**, median days-to-pass 22. Bust attribution: striker 44.4% / aegis 24.1% / guardian 21.3% / NAS 10.2%. Superseded 2026-05-17 by the BT-OFF static-equity canonical (methodology change, not re-lock). See `docs/adr/2026-05-08-dd-trigger-c2-relock.md` (C0→C2 ADR; override grounds may need re-evaluation against BT-OFF panels per the 2026-05-17 reconcile note).
...
* MC at current 4-strategy config (G 0.34% / DJ30 v4.5 1.00% / A 1.50% / NAS v1 0.40%, dd_protection C2 1.5%/0.40×, Pepperstone 2022→2026 BT-OFF panels, 223 week-blocks, 10K × 3 seeds):
  **97.56% pass / 0.40% bust (0.00% daily + 0.40% static) / 2.04% timeout**, p99 DD 4.79%, median days-to-pass 23. Both lock gates clear with margin. Anchor updated 2026-05-17 from prior BT-ON 98.09/0.36/4.73 — see Strategy Reference section above for methodology change details.
